...
const outlineJsonToMarkdownPreview = (raw: string, fallback = '') => {
  ...
  for (const block of itemBlocks) {
    const itemTexts = Array.from(block.matchAll(/"([^"]+)"/g)).map(item => item[1]).filter(Boolean)
  ...
  try {
    ...
    const startLineRender = () => {
      ...
      lineTimer = setInterval(() => {
        ...
        renderedLines.push(pendingLines.shift() as string)
        outline.value = renderedLines.join('\n')
        scrollToBottom()
      }, lineRenderInterval)
    }
    ...
    const flushLines = (force = false) => {
      ...
      lineBuffer = force ? '' : (rest || '')
      if (force && rest) {
        pendingLines.push(rest)
        startLineRender()
      }

      // 没有完整换行时，也实时显示当前行，避免整个区域空白
      if (!force && rest && !pendingLines.length) {
        outline.value = [...renderedLines, rest].join('\n')
        scrollToBottom()
      }
    }
    ...
    const flushEvents = () => {
      ...
      for (const event of events) {
        ...
        try {
          ...
          const nextPreview = outlineJsonToMarkdownPreview(outlineJson, outlinePreview)
          if (nextPreview !== outlinePreview) {
            if (!nextPreview.startsWith(outlinePreview)) {
            ...
            const delta = nextPreview.startsWith(outlinePreview)
              ? nextPreview.slice(outlinePreview.length)
              : nextPreview

            outlinePreview = nextPreview
            lineBuffer += delta.replace(/\r\n/g, '\n')
          }
