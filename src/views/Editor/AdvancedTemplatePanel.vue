...
import { GetPPTGroups, SearchPPTTemplates } from '@/api/editor'
...
const applyTemplate = (item: PPTTemplateView) => {
  if (!item.json) {
  ...
  try {
    const parsed = typeof item.json === 'string'
      ? JSON.parse(item.json) as { slides?: Slide[]; theme?: Partial<SlideTheme> }
      : item.json
